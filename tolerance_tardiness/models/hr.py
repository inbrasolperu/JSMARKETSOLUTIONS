...
class HrAttendance(models.Model):
    ...
    tardiness_minutes = fields.Integer(
        string="Minutos de Tardanza"
    )
    ...
    def create(self, values):
        if values.get('check_in') and values.get('employee_id'):
            ...
            values['tardiness_minutes'] = self.calculate_tardiness_minutes(values['check_in'], values['employee_id'])
    ...
    def write(self, values):
        for rec in self:
            if values.get('check_in') and values.get('employee_id'):
                ...
                values['tardiness_minutes'] = self.calculate_tardiness_minutes(values['check_in'], rec.employee_id.id)
                ...
                values['tardiness_minutes'] = self.calculate_tardiness_minutes(rec.check_in, values['employee_id'])
    ...
    def calculate_tardiness_minutes(self, check_in, employee_id):
        employee_id = self.env['hr.employee'].browse(employee_id)
        check_in = self.convert_date_timezone(check_in)
        schedule = employee_id.resource_calendar_id
        index = str(check_in.weekday())
...
class HrPayslip(models.Model):
    ...
    def get_inputs_data(self):
        ...
        date_temp_from = self.date_from
        date_temp_to = self.date_to
        date_temp_from = date_temp_from.replace(day=1) - timedelta(days=1)
        date_temp_from = date_temp_from.replace(day=1) + timedelta(days=29)
        date_temp_to = date_temp_to.replace(day=1) + timedelta(days=28)
        attendance = self.env['hr.attendance'].search([('employee_id', '=', self.employee_id.id),
                                                       ('check_in', '>=', date_temp_from),
                                                       ('check_in', '<=', date_temp_to)])
        list_minute = [x.tardiness_minutes for x in attendance.sorted('tardiness_minutes')]
        for listminutes in list_minute:
            total_minutes = total_minutes + listminutes
        for result in res:
            if result.get('code') == 'TAR_001':
                result['amount'] = total_minutes
        return res
