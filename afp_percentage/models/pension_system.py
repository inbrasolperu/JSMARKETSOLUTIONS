...
class PensionSystem(models.Model):
    ...
    def button_afp_method(self):
        ...
        self._comision_pension(habitat, pension_line_habitat)
        self._comision_pension(integra, pension_line_integra)
        self._comision_pension(profuturo, pension_line_profuturo)
        self._comision_pension(prima, pension_line_prima)

    def _comision_pension(self, spp, pension_line):
        for i in spp.comis_pension_ids:
            if i.date_from == init_date:
                return
        spp.comis_pension_ids = [(0, 0, pension_line)]
