...
class HrLoan(models.Model):
    ...
    loan_type = fields.Selection([
        ...
        ('quarterly', 'Quarterly'),
        ('yearly', 'Yearly'),
    ], string="Loan type", default='daily', track_visibility='onchange', required=True)
    ...
    def action_approve(self):
        ...
        for request in self:
            ...
            if emp_id:
                ...
                if self.currency_id and self.env.company.currency_id and self.currency_id != self.env.company.currency_id:
                    currency = {'currency_id': self.currency_id.id}
                    debit_dictionary.update(currency)
                debit_line = (0, 0, debit_dictionary)
            ...
            if treasury_id:
                ...
                if self.currency_id and self.env.company.currency_id and self.currency_id != self.env.company.currency_id:
                    currency = {'currency_id': self.currency_id.id}
                    credit_dictionary.update(currency)
                credit_line = (0, 0, credit_dictionary)
