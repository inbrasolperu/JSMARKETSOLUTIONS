...
class HrEmployee(models.Model):
    ...
    account_analytic_tag_ids = fields.Many2one(
        comodel_name='account.analytic.tag',
        string='Etiqueta analítica salariale',
        groups="hr.group_hr_user",
    )


class HRSalaryRuleInhetir(models.Model):
    _inherit = 'hr.salary.rule'

    validated_tags_analytic_employee = fields.Boolean(
        string='Procesar etiquetas analiticas',
        help='Valida que la regla salarial asigne las etiquetas analiticas salariales del empleado. '
             'Recomendacon solo aplicar a cuentas contables de gastos (62).'
    )


class HRPayslipTagAnalytics(models.Model):
    _inherit = 'hr.payslip'


    def _action_create_account_move(self):
        """
        Reescribiendo el metido para asignar las etiquetas analiticas
        """
                ...
                for slip_date in slip_mapped_data[journal_id]:  # For each month.
                    ...
                    for slip in slip_mapped_data[journal_id][slip_date]:
                        ...
                        for line in slip.line_ids.filtered(lambda line: line.category_id):
                            ...
                            # Verificar si procede agregar etiquetas analiticas
                            tag_ids = []
                            analytic_tag_ids = []
                            if slip.employee_id.account_analytic_tag_ids and \
                                    line.salary_rule_id.validated_tags_analytic_employee and \
                                    line.salary_rule_id.account_debit.id == debit_account_id:
                                [
                                    tag_ids.append(tag.id) for tag in slip.employee_id.account_analytic_tag_ids
                                ]
                            ...
                            if debit_account_id:  # If the rule has a debit account.
                                ...
                                if not debit_line:
                                    ...
                                    debit_line['analytic_tag_ids'] = analytic_tag_ids
