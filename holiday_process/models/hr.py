...
from odoo.addons.resource.models.resource import HOURS_PER_DAY, timezone_datetime
import logging
...
class HrPayslip(models.Model):
    ...
    def _get_worked_day_lines(self):
        ...
        if self.contract_id.resource_calendar_id and employee_id.service_termination_date:
            ...
            # VAC_LBS
            vac_lbs_days = sum(line.pending_holiday for line in leaves)
            ...
            vac_lbs_amount = self._get_calc_amount_per_entry(vac_lbs_hours, total_hours, vac_lbs_entry_type_id)

            values = [{
                'sequence': vac_lbs_entry_type_id.sequence, 'work_entry_type_id': vac_lbs_entry_type_id.id, 'number_of_days': vac_lbs_days,
                'number_of_hours': vac_lbs_hours, 'amount': vac_lbs_amount
            }]
            res += values
        return res
