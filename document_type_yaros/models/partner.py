...
class ResPartner(models.Model):
    ...
    def _compute_error_dialog_partner(self):
        latam_company = self.env.company.get_fiscal_country() in [self.env.ref('base.pe'), self.env.ref('base.ar'), self.env.ref('base.cl')]
        for rec in self:
            msg = ''
    ...
    def _onnchange_error_dialog_partner(self):
        for rec in self:
            if rec.error_dialog:
